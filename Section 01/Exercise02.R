# Using edgeR from an ExpressionSet object. Estimating using edgeR from a prepared 'eset' object can be done using the following steps:
count_dataframe <- readr::read_tsv(file.path(getwd(), "datasets", "ch1", "modencodefly_count_table.txt"))
genes <- count_dataframe[['gene']]
count_dataframe[['gene']] <- NULL
count_matrix <- as.matrix(count_dataframe)
rownames(count_matrix) <- genes
phenn_data <- readr::read_table2(file.path(getwd(), "datasets", "ch1", modencodefly_phenodata.txt"))
...
grouping <- pheno_data>% dplyr::filter(stage%in%experiments_of_interest)

counts_of_interest <- count_matrix[, columns_of_interrest]
library("DESeq2")
